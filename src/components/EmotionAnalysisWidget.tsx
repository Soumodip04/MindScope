...
const EmotionAnalysisWidget: React.FC<EmotionAnalysisWidgetProps> = ({ onEmotionDetected }) => {
  ...
  const drawOverlay = useCallback((box: any, angle: number) => {
    ...
    // Only show dynamic elements when face is detected
    if (box) {
      const { x, y, w, h } = box;
      ...
      // 1. Dynamic face detection box (moves with face)
      ctx.save();
      ctx.lineWidth = 3;
      if (aligned) {
      ...
      // 2. Dynamic status message (appears/disappears based on face state)
      if (aligned) {
        ctx.save();
        ctx.font = 'bold 14px system-ui';
        ctx.textAlign = 'center';
        ctx.fillStyle = 'rgba(0, 0, 0, 0.8)';
        ctx.fillRect(cx - 65, 30, 130, 25);
        ctx.fillStyle = 'rgba(0, 255, 120, 1)';
        ctx.fillText('✅ EVERYTHING IS FINE', cx, 47);
        ctx.restore();
      } else {
        // Show positioning hint only when needed
        let hint = '';
        if (sizeRatio <= 0.75) hint = 'Move closer';
        else if (sizeRatio >= 1.35) hint = 'Move back';
        else if (Math.abs(angle) >= 0.2) hint = 'Keep head level';
        ...
        if (hint) {
          ...
          ctx.fillStyle = 'rgba(0, 0, 0, 0.8)';
          ctx.fillRect(cx - hintWidth/2, 30, hintWidth, 25);
          ctx.fillStyle = 'rgba(255, 200, 0, 1)';
          ctx.fillText(hint, cx, 47);
          ctx.restore();
        }
      }
    }
