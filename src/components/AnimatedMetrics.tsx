...
export const AnimatedCounter: React.FC<AnimatedCounterProps> = ({
  ...
  }, [safeValue, duration, controls, motionValue, displayValue, onComplete]);
  ...
  return (
    ...
    >
      {prefix}
      <motion.span>{isNaN(displayValue) ? '0' : displayValue.toFixed(decimals)}</motion.span>
      {suffix}
    </motion.span>
  );
};
        ...
        <div className="flex-1">
          <div className="flex items-center space-x-3 mb-2">
            ...
            <div>
              ...
              {showTrend && previousValue !== undefined && !isNaN(trend) && (
      ...
      <div className="mt-4">
        <div className={`w-full h-2 rounded-full overflow-hidden ${
          ...
          <motion.div
            ...
            initial={{ width: 0 }}
            animate={{ width: `${Math.min(100, isNaN(value) ? 0 : (value / 10) * 100)}%` }}
    ...
    const interval = setInterval(() => {
      setMetrics(prev => {
        ...
        Object.keys(newMetrics).forEach(key => {
          ...
          const safeNewValue = isNaN(newValue) ? metric.current : newValue;
          const safeHeartRateValue = Math.max(60, Math.min(100, metric.current + variation * 5));
          
          newMetrics[key as keyof typeof newMetrics] = {
            previous: metric.current,
            current: key === 'heartRate' 
              ? (isNaN(safeHeartRateValue) ? 72 : safeHeartRateValue)
              : safeNewValue
          };
        });
        ...
        return (
          ...
          >
            <RealTimeMetric
              ...
              decimals={(config as any).decimals || 1}
            />
          </motion.div>
        );
      })}
